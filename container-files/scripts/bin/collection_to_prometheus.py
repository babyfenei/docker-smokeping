...
def pushMetrics(instance , ISP , key , value):
    ...
    metrics = 'smokeping_%s{instance=\"%s\" , ISP=\"%s\" , IDC=\"%s\" , alias=\"%s\"} %d' % (key , instance , ISP , 'SH' , (instance if not paras['province_map'].get(instance) else paras['province_map'].get(instance)) , value)
